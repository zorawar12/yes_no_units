...
mu_x_2 = 0.0                                  #   Mean of distribution from which quality stimulus are sampled randomly
...
if mu_h_vs_mu_x_vs_RCD==1:
    save_string = save_data('mu_h1_muh2_vs_mu_x_vs_RCD')
    ...
    def mux1muh1(muh,mux):
        count = 0
        for k in range(1000):
            success = multi_run(mu_h_1=muh,mu_h_2=2*muh,mu_x_1=mux,mu_x_2=mux,err_type=0)
            if success == 1:
                count += 1
        mu_va = {"mux1":mux,"mux2":mux,"muh1": muh,"muh2": 2*muh,\
             "success_rate":count/1000}
        out = np.array([[mux,mux,muh,2*muh,count/1000]])
...
#    op = pd.read_csv(path+save_string+'last.csv')
#    opt_var = []
#
...
#    graphicPlot(a= y,b=x ,array= opt_var,x_name=r'$\mu_{x}$',y_name=r"$\mu_{h_2}$",z_name="Rate of correct choice",\
#    title="Number_of_options = 10",save_name=path+"1muh2_vs_mu_x_vs_RCD.pdf",z_var=z)

    graphicPlot(a= mu_h,b=mu_x ,array= opt_var,x_name=r'$\mu_{x}$',y_name=r"$\mu_{h_1}$",z_name="Rate of correct choice",title="Number_of_options = 10",save_name=path+save_string+"mux_mu_h1.pdf")

# Majority based Rate of correct choice as a function of sigma_x for varying sigma_h
if sig_h_vs_sig_x_vs_RCD==1:
    save_string = '1sig_h1_vs_sig_x_vs_RCD'  #  save_data('sig_h1_vs_sig_x_vs_RCD')
#    f = open(path+save_string+'.csv','a')
...
#        count = 0
#        for k in range(1000):
#            success = multi_run(sigma_h_1=sigh,sigma_h_2 = 2*sigh,sigma_x_1=sigx,sigma_x_2=sigx,err_type=0)
#            if success == 1:
#                count += 1
#        sig_va = {"sigx1":sigx,"sigx2":sigx,"sigh1": sigh,"sigh2":2*sigh, "success_rate":count/1000}
#        out = np.array([[sigx,sigx,sigh,2*sigh,count/1000]])
#        out = pd.DataFrame(data=out)
#        out.to_csv(path+save_string+'.csv',mode = 'a',header = False, index=False)
#        return sig_va
#
...
#    csv(data=opt_var,file=path+save_string+"last.csv")
    ...
    op = pd.read_csv(path+'1sig_h1_vs_sig_x_vs_RCDlast.csv')
    opt_var = []
    ...
    for i in x:
        ...
        print(np.round(len(z)/(len(x)*len(y)),decimals=2),end="\r")
    print(np.round(len(z)/(len(x)*len(y)),decimals=2))
    graphicPlot(a= y,b=x ,array= opt_var,x_name=r'$\sigma_{x}$',y_name=r"$\sigma_{h_1}$",z_name="Rate of correct choice",\
    title="Number_of_options = 10",save_name=path+save_string+".pdf",z_var=z)

#    graphicPlot(a= sig_h,b=sig_x ,array= opt_var,x_name=r'$\sigma_x$',y_name=r"$\sigma_{h_1}$",z_name="Rate of correct choice",\
