...
mu_x = 0                         #   Lower bound of distribution from which quality stimulus are sampled randomly
sigma_x = 1                         #   Upper bound of distribution from which quality stimulus are sampled randomly
...
sigma_x_vs_sigma_h_vs_RCD = 1
quorum_vs_RC_vs_sigma_m = 0


def units(low_m,high_m,number_of_options):
    a = np.round(np.random.uniform(low=low_m,high=high_m,size=number_of_options),decimals=0)
    for i in range(len(a)):
        while a[i]<=0:
            a[i] = np.round(np.random.uniform(low=low_m,high=high_m),decimals=0)
    return a
...
def one_run(number_of_options=number_of_options,low_m=low_m,high_m=high_m,h_type=h_type,low_h=low_h,high_h=high_h,\
    x_type=x_type,mu_x=mu_x,sigma_x=sigma_x,err_type=err_type,low_assessment_err=low_assessment_err,high_assessment_err=high_assessment_err):
...
def multi_run(number_of_options=number_of_options,low_m=low_m,high_m=high_m,h_type=h_type,low_h=low_h,high_h=high_h,\
    x_type=x_type,mu_x=mu_x,sigma_x=sigma_x,err_type=err_type,low_assessment_err=low_assessment_err,high_assessment_err=high_assessment_err,quorum= None):
...
if mu_x_vs_mu_h_vs_RCD==1:
    save_string ='1mu_x_vs_mu_h_vs_RCD'   #save_data('mu_x_vs_mu_h_vs_RCD')
...
#        count = 0
#        for k in range(1000):
#            success = multi_run(low_h=muh+low_h,high_h=muh+high_h,mu_x=mux,err_type=0)
#            if success == 1:
#                count += 1
...
#    csv(data=opt_var,file = path+save_string+"last.csv")
    op = pd.read_csv(path+'1mu_x_vs_mu_h_vs_RCDlast.csv')
    opt_var = []
    for j in range(len(op['mux'])):
        a ={}
        for i in op:
            a[str(i)] = op[str(i)][j]
        opt_var.append(a)
...
if sigma_x_vs_sigma_h_vs_RCD==1:
    ...
    def highhf(sigh,sigx):
        count = 0
        for k in range(1000):
            success = multi_run(high_h=np.sqrt(3*sigh),low_h=-np.sqrt(3*sigh),sigma_x=sigx,err_type=0)
            if success == 1:
                count += 1
