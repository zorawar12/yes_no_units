...
import uni_class as yn
from multiprocessing import Pool
from operator import itemgetter


number_of_options = 10                      #   Number of options to choose best one from
low_x = -np.sqrt(3)                                  #   Mean of distribution from which quality stimulus are sampled randomly
high_x = np.sqrt(3)                               #   Standard deviation of distribution from which quality stimulus are sampled randomly
low_h = -np.sqrt(3)                                    #   Mean of distribution from which units threshold are sampled randoml
high_h = np.sqrt(3)                               #   Standard deviation of distribution from which units threshold are sampled randomly
low_m = 100                                 #   Mean of distribution from which number of units to be assigned to an option are sampled randomly
high_m = 100                                 #   Standard deviation of distribution from which number of units to be assigned to an option are sampled randomly
...
if success_rate_sigma_h_number_options==1:
    ...
    def sighf(op,sigh):
        count = 0
        for k in range(1000):
            success = multi_run(high_h=sigh,low_h=-sigh,number_of_options=op,err_type=0)
            if success == 1:
                count += 1
    ...
    plt_show(data_len= opts,array= opt_var,var= "opt", plt_var="sigh",x_name=r'$\sigma_h$',\
        title="Number of options",save_name="sigma_h_vs_RCD_vs_nop.pdf")
...
if success_rate_mu_h_number_options==1:
    ...
    plt_show(data_len= opts,array= opt_var,var= "opt", plt_var="mu_h",x_name=r'$\mu_h$',\
...
if success_rate_number_options_mu_h==1:
    ...
    plt_show(data_len= mu_h,array= opt_var,var= "muh", plt_var="nop",x_name='Number_of_options',\
        title=r"$\mu_h$",save_name="nop_vs_RCD_vs_mu_h.pdf")
...
if success_rate_mu_m_number_options==1:
    ...
    def lowmf(nop,mum):
        count = 0
        for k in range(2000):
            success = multi_run(low_m=mum,high_m= mum,number_of_options=nop,err_type=0)
            if success == 1:
                count += 1
        nop_va = {"nop":nop,"mum": mum, "success_rate":count/2000}
        return nop_va

    opt_var = parallel(lowmf,number_of_options,mu_m)

    plt_show(data_len= number_of_options,array= opt_var,var= "nop", plt_var="mum",x_name=r'$\mu_m$',\
...
if success_rate_mu_m_number_options==1:
    ...
    def highmf(nop,mum):
        count = 0
        for k in range(2000):
            success = multi_run(high_m=mum+high_h,low_m=mum+low_m,number_of_options=nop,err_type=0) 
            if success == 1:
                count += 1
        nop_va = {"nop":nop,"mum": mum, "success_rate":count/2000}
        return nop_va
    ...
    plt_show(data_len= number_of_options,array= opt_var,var= "nop", plt_var="mum",x_name= r'$\mu_m(\sigma_m \neq 0)$',\
...
if success_rate_sigma_h_mu_h==1:
    ...
    def highhf(muh,sigmah):
        count = 0
        for k in range(3000):
            success = multi_run(low_h=muh+low_h-sigmah,high_h=muh+high_h+sigmah,err_type=0)
            if success == 1:
                count += 1
    ...
    graphic_plt(a= mu_h,b=sigma_h ,array= opt_var,x_name=r'$\sigma_h$',y_name=r"$\mu_h$",title="Number_of_options = 10",\
...
if success_rate_mu_x_mu_h==1:
    ...
    graphic_plt(a= mu_h,b=mu_x ,array= opt_var,x_name=r'$\mu_x$',y_name=r"$\mu_h$",title="Number_of_options = 10",\
...
if success_rate_sigma_x_sigma_h==1:
    ...
    def highhf(sigh,sigx):
        count = 0
        for k in range(1000):
            success = multi_run(high_h=sigh+high_h,high_x=sigx+high_x,low_h=low_h-sigh,low_x=low_x-sigx,err_type=0)
            if success == 1:
                count += 1
    ...
    graphic_plt(a= sigma_h,b=sigma_x ,array= opt_var,x_name=r'$\sigma_x$',y_name=r"$\sigma_h$",title="Number_of_options = 10",\
...
if rate_of_choice_quorum_sigma_m==1:
    ...
    def quof(sigm,quo):
        ...
        for k in range(1000):
            correct,quorum_reached,majority_dec = multi_run(high_m=sigm+high_m,low_m=low_m-sigm,quorum=quo,err_type=0)
            right_count += correct
            if quorum_reached == 0:
                q_not_reached += 1
    ...
    save_name = ["quorum_high_m"+str(i)+".pdf" for i in sigma_m]
    opt_v = {}
    prev = None
    for i in opt_var:
        if i["sigm"] == prev:
    ...
    for i in range(len(save_name)):
        bar(quorum,opt_v[str(sigma_m[i])],save_name[i],"maj")
