...
def parallel(func,a,b):
    ...
    with Pool(8) as p:#24,ray_address="auto") as p:
...
if mu_h_vs_mu_x_vs_RCD==1:
    ...
    mu_x = [np.round(-4.0+i,decimals=1) for i in range(50)]
    mu_h = [np.round(-4.0+i,decimals=1) for i in range(50)]
    ...
    data_visualize(file_name="mu_h_1_vs_mu_x_1_vs_RCD"+"last.csv",save_plot="mu_h_1_vs_mu_x_1_vs_RCD",x_var_='$\mu_{x_1}$',y_var_='$\mu_{h_1}$',cbar_orien="horizontal",data=opt_var1)
