...
mu_h = 0                         #   Lower bound of distribution from which units threshold are sampled randoml
sigma_h = 1                         #   Upper bound of distribution from which units threshold are sampled randomly
...
def threshold(m_units,h_type,mu_h,sigma_h):
    return np.round(np.random.normal(mu_h,sigma_h,size=abs(m_units)),decimals=h_type)
...
def one_run(number_of_options=number_of_options,low_m=low_m,high_m=high_m,h_type=h_type,mu_h=mu_h,sigma_h=sigma_h,\
...
def multi_run(number_of_options=number_of_options,low_m=low_m,high_m=high_m,h_type=h_type,mu_h=mu_h,sigma_h=sigma_h,\
