...
def linePlot(data_len,array,var,plt_var,x_name,title,save_name):
    ...
    plt.xlabel(x_name)
    plt.ylabel('Rate of correct choice')
...
if With_assesment_error_Majority_based_decision==1:
    one_run(lambda_assessment_err=1.0)
...
if mu_h_vs_sigma_h_vs_RCD_vs_nop==1:
    sig_h = [(0.0+i*0.01) for i in range(201)]	#	sigma_h = mu_h
    opts = [2,10,40,60]

    def highf(op,sigh):
        count = 0
        for k in range(1000):
            success = multi_run(lambda_h=sigh,number_of_options=op,err_type=0)
            if success == 1:
                count += 1
        opt_va = {"opt":op,"sigh": sigh, "success_rate":count/1000}
        return opt_va

    opt_var = parallel(highf,opts,sig_h)
    csv(data=opt_var,file = path+"mu_h_vs_sigma_h_vs_RCD_vs_nop.csv")
    linePlot(data_len= opts,array= opt_var,var= "opt", plt_var="sigh",x_name=r'$\sigma_h$',\
    title="Number of options",save_name=path+"mu_h_vs_sigma_h_vs_RCD_vs_nop.pdf")
...
if nop_vs_RCD_vs_mu_h==1:
    ...
    def nopf(muh,nop):
        count = 0
        for k in range(2000):
            success = multi_run(lambda_h=muh,number_of_options=nop,err_type=0)
            if success == 1:
                count += 1
    ...
    title=r"$\mu_h$",save_name=path+"nop_vs_RCD_vs_mu_h.pdf")

# Majority based Rate of correct choice as a function of mu_m for varying number of options 
if mu_m_vs_RCD_vs_nop==1:
    mu_m = [i for i in range(1,101,1)]	#	mu_m = sigma_m
    number_of_options = [2,10]

    def lowmf(nop,mum):
        count = 0
        for k in range(2000):
            success = multi_run(lambda_m=mum,number_of_options=nop,err_type=0)
            if success == 1:
                count += 1
    ...
    title="Number_of_options",save_name=path+"mu_m_vs_RCD_vs_nop.pdf")

# Majority based Rate of correct choice as a function of mu_x for varying mu_h
if mu_x_vs_mu_h_vs_RCD==1:
    mu_x = [np.round(0.0+i*0.08,decimals=2) for i in range(101)]
    mu_h = [np.round(0.0+i*0.08,decimals=2) for i in range(101)]

    def lowhxf(muh,mux):
        count = 0
        for k in range(1000):
            success = multi_run(lambda_h=muh,lambda_x=mux,err_type=0)
            if success == 1:
                count += 1
...
if quorum_vs_RCD_vs_sigma_m==1:
    ...
    def quof(sigm,quo):
        ...
        for k in range(1000):
            correct,quorum_reached,majority_dec = multi_run(lambda_m=sigm,quorum=quo,err_type=0)
            right_count += correct
            if quorum_reached == 0:
                q_not_reached += 1
    ...
    csv(data=opt_var,file = path+ "quorum_vs_RCD_vs_sigma_m.csv")
    save_name = [path+"quorum_vs_RCD_vs_sigma_m"+str(i)+".pdf" for i in sigma_m]
    opt_v = {}
    prev = None
    for i in opt_var:
        if i["sigm"] == prev:
