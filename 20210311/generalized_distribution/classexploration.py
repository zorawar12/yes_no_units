    ...
    def data_visualize(self,file_name,save_plot,x_var_,y_var_,z_var_,plot_type,gaussian=1,uniform=0,cbar_orien=None,line_labels=None,sigma_x_1=None,\
        ...
        if plot_type == 'graphics':
            ...
            if gaussian ==1:
                wf = workFlow()
                # Method 1
                [x_1,x_2] = wf.ESM_onebyn_method1(delta_mu,x_var_,x,sigma_x_1,sigma_x_2,line_labels,wf.gaussian,wf.ICPDF)
                # [x_1,x_2] = wf.ESM_oneby2n_method2(delta_mu,x_var_,x,sigma_x_1,sigma_x_2,line_labels,wf.gaussian,wf.ICPDF)
                ...
                self.graphicPlot(a= y,b=x,array= opt_var,x_name=r'%s'%x_var_,y_name=r'%s'%y_var_,z_name=z_var_,title="Number_of_options = "+str(num_of_opts),save_name=path+save_plot+x_var_[2:-1]+y_var_[2:-1]+'RCD_method3'+'delta_m'+str(abs(m_3-m))+'d'+str(d_3)+'.pdf',cbar_loc=cbar_orien,z_var=z,options_line=[[m_3,b_3]],line_labels=[line_labels[0]],z_max_fit = [m,b])
            ...
            if uniform ==1:
                ...
                # m_1, b_1 = np. polyfit(x, x_1, 1)
                # d_1 = abs(b_1-b)/ np.sqrt(m**2 +1)
                
                # self.graphicPlot(a= y,b=x,array= opt_var,x_name=r'%s'%x_var_,y_name=r'%s'%y_var_,z_name=z_var_,title="Number_of_options = "+str(num_of_opts),save_name=path+save_plot+x_var_[2:-1]+y_var_[2:-1]+'RCD_method1'+'delta_m'+str(abs(m_1-m))+'d'+str(d_1)+'.pdf',cbar_loc=cbar_orien,z_var=z,z_max_fit = [m,b],options_line=[x_1,x_2],line_labels=line_labels)
                # self.graphicPlot(a= y,b=x,array= opt_var,x_name=r'%s'%x_var_,y_name=r'%s'%y_var_,z_name=z_var_,title="Number_of_options = "+str(num_of_opts),save_name=path+save_plot+x_var_[2:-1]+y_var_[2:-1]+'RCD.pdf',cbar_loc=cbar_orien,z_var=z_best,z_max_fit = [m,b],options_line=[x_1,x_2],line_labels=line_labels)
                ...
                #     step = 0.0001
                #     dis_x = np.round(np.arange(start,stop,step),decimals=6)
                #     pdf =  [wf.uniform(i,mu,sigma) for i in dis_x]
                #     pdf = np.multiply(pdf,1/(np.sum(pdf)*step))
                #     for area in areas:
                #         # print(area)
                #         inverse_P =  wf.ICPDF(area**(1/line_labels[0]),mu,sigma,start,stop,step,dis_x,pdf)
                ...
                # m_3, b_3 = np. polyfit(x, expected_highest_quality, 1)

                # d_3 = abs(b_3-b)/ np.sqrt(m**2 +1)

                # self.graphicPlot(a= y,b=x,array= opt_var,x_name=r'%s'%x_var_,y_name=r'%s'%y_var_,z_name=z_var_,title="Number_of_options = "+str(num_of_opts),save_name=path+save_plot+x_var_[2:-1]+y_var_[2:-1]+'RCD_method3'+'delta_m'+str(abs(m_3-m))+'d'+str(d_3)+'.pdf',cbar_loc=cbar_orien,z_var=z,options_line=[expected_highest_quality],line_labels=[line_labels[0]],z_max_fit = [m,b])
