...
class workFlow:
    ...
    def one_run(self,number_of_options,mu_m,sigma_m,mu_h,sigma_h,\
        ...
        pc = rng.units(number_of_options=number_of_options,mu=[mu_m],sigma=[sigma_m])

        units_distribution = []
        
        for i in pc:
            units_distribution.append(rng.threshold(m_units=i,mu_h=[mu_h],sigma_h=[sigma_h]))
        
        ref,qc = rng.quality(number_of_options=number_of_options,mu_x=[mu_x],sigma_x=[sigma_x])

        dec = self.majority_decision(number_of_options=number_of_options,Dx = qc,assigned_units= units_distribution,\
            err_type=err_type,mu_assessment_err= mu_assessment_err,sigma_assessment_err=sigma_assessment_err,\
            ref_highest_quality=ref,quorum=quorum)
