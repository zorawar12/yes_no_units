...
@njit
def units(mu,sigma,number_of_options):
      a = np.zeros(number_of_options)
      peak_choice = np.random.randint(0,len(mu),number_of_options)
      for i in range(len(peak_choice)):
            k = int(np.random.normal(mu[peak_choice[i]],sigma[peak_choice[i]]))
            while k<=0:
                  k = int(np.random.normal(mu[peak_choice[i]],sigma[peak_choice[i]]))
            a[i] = k      
      return a
...
@njit
def threshold(mu_h,sigma_h,m_units,h_type=3):
      a = np.zeros(m_units)
...
@njit
def dx(mu_x,sigma_x,number_of_options,x_type=3):
      """
...
def quality(mu_x,sigma_x,number_of_options,x_type=3):
      dis_x = dx(mu_x,sigma_x,number_of_options)
