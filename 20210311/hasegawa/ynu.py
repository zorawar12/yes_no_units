...
def parallel(func,a,b,batch_size,save_string,columns_name):
    ...
    for i in range(0,len(inp),batch_size):
        ...
        progress +=1
        print("\r Percent of input processed : {}%".format(np.round(100*progress/int(len(inp)/batch_size)),decimals=1), end="")
...
if sig_h_vs_RCD_vs_nop==1:
    ...
    runs = 200
    sig_h = [i*0.1 for i in range(200)]
    opts = [2,10,40]
    mu_m = 100
    sigma_m = 0
    mu_h = 0
    ...
    parallel(sighf,opts,sig_h,columns_name=['$\sigma_{h}$','$opts$','success_rate'],save_string=save_string,batch_size=int(len(sig_h)/5))
