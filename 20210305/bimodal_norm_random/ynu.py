...
def one_run(number_of_options=number_of_options,mu_m_1=mu_m_1,sigma_m_1=sigma_m_1,\
    ...
    pc = np.array(units(number_of_options=number_of_options,mu_m_1=mu_m_1,sigma_m_1=sigma_m_1,mu_m_2=mu_m_1,sigma_m_2=sigma_m_1)).astype(int)

    units_distribution = []
    for i in pc:
        thresholds = threshold(m_units = int(i/2) ,h_type=h_type,mu_h_1=mu_h_1,sigma_h_1=sigma_h_1,mu_h_2=mu_h_1,sigma_h_2=sigma_h_1)
...
if sig_h_vs_sig_x_vs_RCD==1:
    # save_string = save_data('sig_h1_vs_sig_h2_vs_sig_x_vs_RCD')
    # f = open(path+save_string+'.csv','a')
    # column = column = pd.DataFrame(data = np.array([["$\sigma_{x_1}$","$\sigma_{x_2}$","$\sigma_{h_1}$","$\sigma_{h_2}$","success_rate"]]))
    # column.to_csv(path+save_string+'.csv',mode='a',header= False,index=False)
    ...
    #         if success == 1:
    #             count += 1
    #     sig_va = {"$\sigma_{x_1}$":sigx,"$\sigma_{x_2}$":sigx,"$\sigma_{h_1}$": sigh,"$\sigma_{h_2}$":2+sigh, "success_rate":count/1000}
    ...
    # csv(data=opt_var,file=path+save_string+"last.csv")
    data_visualize(file_name='1sig_h1_vs_sig_h2_vs_sig_x_vs_RCDlast.csv',save_plot='1constant',x_var_='$\sigma_{x_2}$',y_var_='$\sigma_{h_2}$',cbar_orien="vertical")
