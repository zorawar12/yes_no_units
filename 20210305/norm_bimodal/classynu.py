...
class qualityControl:
    ...
    def dx(self):
        ...
        """
        if self.number_of_options%2 == 0:
            dx1 = np.round(np.random.normal(self.mu_x_1,self.sigma_x_1,int(self.number_of_options/2)),decimals=self.x_type)
            dx2 = np.round(np.random.normal(self.mu_x_2,self.sigma_x_2,int(self.number_of_options/2)),decimals=self.x_type)
            self.Dx = np.concatenate((dx1,dx2),axis=None)
        else:
            dx1 = np.round(np.random.normal(self.mu_x_1,self.sigma_x_1,int((self.number_of_options+1)/2)),decimals=self.x_type)
            dx2 = np.round(np.random.normal(self.mu_x_2,self.sigma_x_2,int((self.number_of_options-1)/2)),decimals=self.x_type)
            self.Dx = np.concatenate((dx1,dx2),axis=None)
