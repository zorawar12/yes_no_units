...
class workFlow:
    ...
    def finding_gaussian_base(self,area,mu,sigma):
        ...
        while abs(dummy_area-area)>0.00001:
            print([dummy_area,area])
            x_.append(x)
            fx = self.gaussian(x,mu,sigma)
            fx_.append(fx)
            dummy_area += multiplier*fx*step
            x += step
        print(dummy_area)
        return [x,x_,fx_]
...
class Visualization:
    def data_visualize(self,file_name,save_plot,x_var_,y_var_,z_var_,plot_type,cbar_orien=None,line_labels=None,sigma_x_1=None,\
        ...
        if plot_type == 'graphics':
            ...
            x_2 = []
            for i in x:
                [_1,intermediate_x,intermediate_fx] = wf.finding_gaussian_base(1-(1/line_labels[0]),[i,i+delta_mu],[sigma_x_1,sigma_x_2])
                [_2,intermediate_x,intermediate_fx] = wf.finding_gaussian_base(1-(1/line_labels[1]),[i,i+delta_mu],[sigma_x_1,sigma_x_2])
                x_1.append(_1)
                x_2.append(_2)
